...
def test_decide_done():
    def call(system, user):
        return {"status": "done", "answer": "", "url": "", "goals": [], "clarify": ""}

    page = {"url": "https://app.warmy.io", "title": "Warmy", "text": "inbox 14 healthy"}
    assert decide("check inbox 14", page, [], call_llm=call) == {"status": "done", "answer": ""}


def test_decide_done_with_answer():
    def call(system, user):
        return {"status": "done", "answer": "You have 3 replies.", "url": "", "goals": [], "clarify": ""}

    page = {"url": "https://app.warmy.io", "title": "Warmy", "text": "3 replies"}
    out = decide("how many replies", page, [], call_llm=call)
    assert out == {"status": "done", "answer": "You have 3 replies."}


def test_decide_continue_and_clarify():
    def call(system, user):
        return {"status": "continue", "answer": "", "url": "", "goals": ["open inbox 2"], "clarify": ""}
    ...
    def ambiguous(system, user):
        return {"status": "clarify", "answer": "", "url": "", "goals": [], "clarify": "which campaign?"}
